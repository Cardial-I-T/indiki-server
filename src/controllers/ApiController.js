...
function ApiController() {
  ...
  async function localizar(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ errorMessage: "Erro ao buscar usuário", error: error.message });
    }
  }
  ...
  async function cadastrar(req, res) {
    ...
    try {
      await modelUsuario.criarUsuario(
        nome,
        email,
        senha,
        data_aniversario,
      ...
      console.log("Criou?", criarUsuario);
  ...
  async function atualizar(req, res) {
    ...
    const { admin, nome, senha, email, data_aniversario, cnpj, empresa } =
      req.body;

    try {
      await modelUsuario.atualizarUsuario(
        cpf,
        admin,
        nome,
        senha,
  ...
  async function excluir(req, res) {
    ...
    try {
      const numRegistrosExcluidos = await modelUsuario.excluirUsuario(
        usuario_id
      );
      if (numRegistrosExcluidos === 0) {
        res
          .status(404)
          .json({ message: `Usuário ${usuario_id} não encontrado` });
      } else {
